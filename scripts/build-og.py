...
문구를 바꾸려면 맨 아래 build() 인자만 고치고 실행한다.
필요한 가변 폰트는 저장소에 두지 않는다. 실행 전 같은 폴더에 내려받을 것:
...
def base_image():
    ...
    top = round(im.height * 0.30)
    im = im.crop((0, top, W, top + H))

    layer = Image.new("RGBA", (W, H), NAVY + (0,))
    px = layer.load()
    for x in range(W):
        a = 1.0 if x <= GRAD_SOLID else 1 - ease((x - GRAD_SOLID) / GRAD_FADE)
        if a <= 0:
            continue
        v = NAVY + (round(255 * (0.10 + 0.90 * a)),)   # 오른쪽도 살짝 눌러 통일감
        for y in range(H):
            px[x, y] = v
